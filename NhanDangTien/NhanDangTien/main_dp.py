    ...
    for result in results:
        ...
        for box in boxes:
            ...
            if confidence >= confidence_threshold:
                ...
                cv2.putText(image, label, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255    , 0), 2)
    return image
...
combo = ttk.Combobox(control_frame, values=combo_options, width=25)
combo.set(combo_options[0])
combo.pack()
confirm_button = tk.Button(control_frame, text="XÁC NHẬN", command=confirm_mode, width=20, bg="red", fg="white")
confirm_button.pack(pady=5)

# Frame cho cài đặt camera
video_frame = tk.Frame(control_frame)
camera_label = tk.Label(video_frame, text="Camera :")
camera_label.pack()
camera_options = list_cameras()
camera_combo = ttk.Combobox(video_frame, values=camera_options, width=25)
if camera_options:
    camera_combo.set(camera_options[0])
camera_combo.pack()
start_button = tk.Button(video_frame, text="BẮT ĐẦU", width=20, command=start_recognition, bg="green", fg="white")
start_button.pack(pady=5)
stop_button = tk.Button(video_frame, text="DỪNG", width=20, command=stop_recognition, bg="red", fg="white")
stop_button.pack(pady=5)
trigger_button = tk.Button(video_frame, text="KÍCH HOẠT NHẬN DẠNG", width=20, command=trigger_recognition_toggle, bg="yellow", fg="black")
trigger_button.pack(pady=5)

# Frame cho chọn ảnh
picture_frame = tk.Frame(control_frame)
path_label = tk.Label(picture_frame, text="Đường dẫn :")
path_label.pack()
path_entry = tk.Entry(picture_frame, width=30, state='readonly')
path_entry.pack()
choose_button = tk.Button(picture_frame, text="CHỌN", width=20, command=open_image, bg="red", fg="white")
choose_button.pack(pady=5)
result_button = tk.Button(picture_frame, text="KẾT QUẢ", width=20, command=result_image, bg="green", fg="white")
...
confidence_label = tk.Label(control_frame, text="Độ tin cậy :")
confidence_label.pack()
confidence_value = tk.DoubleVar(value=0.5)
confidence_spinbox = ttk.Spinbox(control_frame, from_=0, to=1, increment=0.1, textvariable=confidence_value, width=10)
confidence_spinbox.pack()

save_button = tk.Button(control_frame, text="LƯU", width=20, command=update_confidence_threshold)
save_button.pack(pady=10)

refresh_button = tk.Button(control_frame, text="LÀM MỚI", width=20, command=refresh_app, bg="blue", fg="white")
refresh_button.pack(pady=10)

exit_button = tk.Button(control_frame, text="THOÁT", width=20, command=root.quit, bg="red", fg="white")
